...
while ($hasil=mysqli_fetch_assoc($query)) {
	...
	echo "<tr>
			...
			<td><a href='?p=30&kode=$hasil[kode_alat]' style='TEXT-DECORATION: NONE'>$hasil[kode_alat]</a></td>
			<td>$hasil[nama]</td>
			<td>$hasil[merek]</td>
			<td>$hasil[model]</td>
			<td>$tgl_beli</td>
...
function tampil_kategori($a){
	...
	$nomer=0;
	$query=mysqli_query($conn,"SELECT kode_alat, nama ,merek,model,tgl_beli,status, 
	id_ruang,nama_ruang FROM alat,jenis,ruang where (substring(kode_alat,1,2)='$a' and  
	substring(kode_alat,3,2)=kode) and id_ruang=ruang.id ORDER BY kode_alat" );
	while ($hasil=mysqli_fetch_assoc($query)) {
		$nomer++;
		$tgl_beli = explode("-", $hasil['tgl_beli']);
...
function tampil_jenis($a,$b){
	...
	$query=mysqli_query($conn,"SELECT kode_alat, nama ,merek,model,tgl_beli,status, 
	id_ruang,nama_ruang FROM alat,jenis,ruang where substring(kode_alat,1,2)='$a' and  
	substring(kode_alat,3,2)='$b' and (substring(kode_alat,1,2)=kd and substring(kode_alat,3,2)=kode) 
	and id_ruang=ruang.id ORDER BY kode_alat DESC");
	while ($hasil=mysqli_fetch_assoc($query)) {
		$nomer++;
		$tgl_beli = explode("-", $hasil['tgl_beli']);
		$tgl_beli = $tgl_beli[2]."-".$tgl_beli[1]."-".$tgl_beli[0];
